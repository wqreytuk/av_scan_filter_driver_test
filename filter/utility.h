    ...
    )
{
    ...
    //
	PVOID abc = ExAllocatePoolWithTagPriority(NonPagedPoolNx,
		sizeof(ERESOURCE),
		AV_RESOURCE_TAG, NormalPoolPriority);
	RtlZeroMemory(
		abc, sizeof(ERESOURCE));
	return (PERESOURCE)abc;
}
    ...
    )
{
	...
	PVOID abc = ExAllocatePoolWithTagPriority(NonPagedPoolNx,
		sizeof(KEVENT),
		AV_KEVENT_TAG, NormalPoolPriority);
	RtlZeroMemory(
		abc, sizeof(KEVENT));
	return (PKEVENT)abc; 
}
