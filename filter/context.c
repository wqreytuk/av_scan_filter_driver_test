...
--*/
{
    ...
    for (;;) {
		...
		PVOID abccoinstArraynnectionCookie = ExAllocatePoolWithTagPriority(PagedPool,
			(instCnt * sizeof(PFLT_INSTANCE)),
			AV_INSTANCES_ARRAY_TAG, NormalPoolPriority);
		RtlZeroMemory(
			abccoinstArraynnectionCookie, instCnt * sizeof(PFLT_INSTANCE));
		instArray = (PFLT_INSTANCE)abccoinstArraynnectionCookie;
...
--*/
{
	...
	PVOID abccoinsscanCtxtArraynnectionCookie = ExAllocatePoolWithTagPriority(NonPagedPoolNx,
		sizeof(AV_SCAN_CONTEXT),
		AV_SCAN_CTX_TAG, NormalPoolPriority);
	RtlZeroMemory(
		abccoinsscanCtxtArraynnectionCookie, sizeof(AV_SCAN_CONTEXT));
	scanCtx = (PAV_SCAN_CONTEXT)abccoinsscanCtxtArraynnectionCookie;
